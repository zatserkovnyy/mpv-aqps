...
local BASE_AUDIO_BITRATES = {
	...
	alac = 1.200 / 2,
	pcm = 1.536 / 2,
	dts = { stereo = 0.768, multichannel = 1.509 },
	eac3 = 0.640,
	ac3 = 0.448,
	mp3 = 0.192,
	vorbis = 0.192,
...
local function get_video_bitrate_and_fps(path)
	...
	local res, err = utils.subprocess({
		args = {
			...
			"v:0",
			"-show_entries",
			"stream=bit_rate,r_frame_rate:stream_tags=BPS",
			"-of",
			"json",
			path,
		},
...
local function determine_quality_profile(avg_bitrate, height)
	...
	if category == "2160p" then
		profile = q_bitrate >= BITRATE_2160_HQ and "2160p-HQ"
			or q_bitrate >= BITRATE_2160_MQ and "2160p-MQ"
			or "2160p-LQ"
	elseif category == "1080p" then
		profile = q_bitrate >= BITRATE_1080_HQ and "1080p-HQ"
			or q_bitrate >= BITRATE_1080_MQ and "1080p-MQ"
			or "1080p-LQ"
	elseif category == "720p" then
...
local function apply_video_quality_profile()
	...
	if profile ~= "" and profile ~= "Default" then
		...
		if #coeff_parts > 0 then
			...
			full_profile_str = string.format(
				"%s%s [%s%s Mbps @ %s] → [%s%s Mbps @ %s]",
				profile,
				osd_hdr_text,
				prefix,
				x_text,
			...
			full_profile_str =
				string.format("%s%s [%s%s Mbps @ %s]", profile, osd_hdr_text, prefix, x_text, video_bitrate_source)
		end
...
local function get_readable_audio_codec_name(track)
	...
	if c_lower:find("dts") or c_lower:find("dca") or ao_format:find("dts") or decoder_name:find("dts") then
		...
		if
			info:find("master audio")
			or info:find("hdma")
			or info:find("dts%-ma")
			or info:find("dtshd%-ma")
			or c_profile:find("ma")
		then
			res = "DTS-HD Master Audio"
		elseif
			info:find("high resolution")
			or info:find("dts%-hr")
			or info:find("dtshd%-hr")
			or c_profile:find("hr")
		then
...
local function generate_static_osd_info()
	...
	elseif avg_bitrate then
		if profile == "hdtv" then
			state.osd_profile_line =
				string.format("Profile: %s [%s%s Mbps @ %s]", string.upper(profile), prefix, bitrate_display, video_bitrate_source)
		else
			...
			if #coeff_parts > 0 then
				local coeff_str = table.concat(coeff_parts, ", ")
				state.osd_profile_line = string.format(
					"Profile: %s%s [%s%s Mbps @ %s] → [%s%s Mbps @ %s]",
					profile,
					state.osd_hdr_text,
					prefix,
					x_text,
				...
				state.osd_profile_line =
					string.format("Profile: %s%s [%s%s Mbps @ %s]", profile, state.osd_hdr_text, prefix, x_text, source)
			end
		end
	else
	...
	if deband and deband ~= "no" then
		state.osd_deband_line = string.format(
			"Debanding: iterations=%s, threshold=%s, range=%s, grain=%s",
			mp.get_property_number("deband-iterations") or "-",
			mp.get_property_number("deband-threshold") or "-",
			mp.get_property_number("deband-range") or "-",
			mp.get_property_number("deband-grain") or "-"
		)
	else
	...
	state.osd_video_line_prefix = string.format(
		"Video: %dx%d%s / %s / %s (%s) [",
		dw,
		dh,
		state.osd_fps_display,
...
local function generate_dynamic_osd_info()
	...
	if sel_audio then
		...
		local chan_text = ch == 8 and "7.1"
			or ch == 6 and "5.1"
			or ch == 2 and "2.0"
			or ch == 1 and "1.0"
			or tostring(ch) .. "ch"

		local abps_kbps_text = (state.audio_bitrate_kbps and state.audio_bitrate_kbps > 0)
				and string.format("%d kbps", math.floor(state.audio_bitrate_kbps))
			or "n/a"

		state.osd_audio_line = string.format(
			"Audio: [%s]%s | (%s %s @ %s) [%d/%d]",
			(sel_audio.lang or "und"):upper(),
			sel_audio.title and (" " .. sel_audio.title) or "",
			get_readable_audio_codec_name(sel_audio),
			chan_text,
			abps_kbps_text,
			sel_audio_idx,
			audio_cnt
	...
	if sel_sub then
		state.osd_subs_line = string.format(
			"Subs: [%s]%s | %s [%d/%d]",
			(sel_sub.lang or "und"):upper(),
			sel_sub.title and (" " .. sel_sub.title) or "",
			get_readable_subtitle_type(sel_sub),
			sel_sub_idx,
			sub_cnt
		)
	end
...
local function display_osd()
	...
	local v_bitrate_text = state.video_bitrate and string.format("%.1f", math.floor(state.video_bitrate * 10) / 10)
		or "n/a"
	...
	local time_line = string.format(
		"%s | %s [%d%%] %s (-%s) | %s",
		curr_time,
		format_time_hms(time_pos, true, true),
		progress,
		format_time_hms(duration, true, true),
		format_time_hms(duration - time_pos, true, true),
		exit_time
	)
